...
const CITY_COORDS = {
  'Shanghai (CNSHA)': { lat: 31.2304, lng: 121.4737, nameKo: '상하이', nameEn: 'Shanghai' },
  'Busan (KRPUS)': { lat: 35.1028, lng: 129.0403, nameKo: '부산', nameEn: 'Busan' },
  'Rotterdam (RTM)': { lat: 51.9244, lng: 4.4777, nameKo: '로테르담', nameEn: 'Rotterdam' },
  'Incheon (ICN)': { lat: 37.4602, lng: 126.4407, nameKo: '인천', nameEn: 'Incheon' },
  'Los Angeles (LAX)': { lat: 33.7423, lng: -118.2705, nameKo: '로스앤젤레스', nameEn: 'Los Angeles' },
  'Frankfurt (FRA)': { lat: 50.0379, lng: 8.5622, nameKo: '프랑크푸르트', nameEn: 'Frankfurt' }
};
...
export const CargoRegisterCard = ({ selectedShipment, onSelectShipment }) => {
  ...
  const getWeatherDescription = (code) => {
    if (lang === 'en') {
      ...
      if (code >= 51 && code <= 67) return '🌧️ Light Rain';
      if (code >= 71 && code <= 77) return '❄️ Snow';
      if (code >= 80 && code <= 82) return '🌧️ Heavy Rain';
      if (code >= 95) return '🌩️ Marine Thunderstorm';
  ...
  const handleSubmit = (e) => {
    ...
    const newShipment = {
      ...
      progress_pct: 15,
      current_step: lang === 'en' ? 'Registered & Preparing' : '등록 완료 및 출항 준비 중',
      original_eta: formData.original_eta,
      current_eta: vesselRealtimeEta || formData.original_eta,
      delay_reason: `Port Congestion (${portCongestionPct}%) & Open-Meteo Weather Applied`,
  ...
  const destCityObj = CITY_COORDS[formData.destination];
  const destCityName = lang === 'en' ? (destCityObj?.nameEn || 'Destination') : (destCityObj?.nameKo || '도착지');
        ...
        <div className="form-grid-2col">
          ...
          <div className="form-group" style={{ marginBottom: 0 }}>
            <label className="form-label" style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: '#334155', fontSize: '0.78rem', fontWeight: 700, marginBottom: '0.25rem' }}>
              ...
              <span>{t('transportModeLabel')}</span>
            </label>
            <select 
          ...
          <div style={{ background: '#fffbe6', border: '1px solid #fef08a', borderRadius: 'var(--radius-md)', padding: '0.75rem 0.8rem' }}>
            ...
            <div style={{ background: '#ffffff', border: '1px solid #fde68a', borderRadius: 'var(--radius-sm)', padding: '0.55rem 0.65rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.2rem', fontSize: '0.72rem', color: '#334155' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                    <Anchor className="w-3 h-3 text-blue-600" /> {t('portCongestionLabel')}
                  </span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
                    <Calendar className="w-3 h-3 text-rose-600" /> {t('vesselEtaLabel')}
                  </span>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.2rem', marginTop: '0.2rem', borderTop: '1px dashed #fef08a' }}>
                  <span style={{ display: 'flex', alignItems: 'center', gap: '0.25rem', color: '#0284c7', fontWeight: 700 }}>
                    <CloudSun className="w-3 h-3 text-sky-600" /> Open-Meteo ({destCityName}):
                  </span>
                  <strong style={{ color: '#0369a1', fontSize: '0.68rem' }}>
                    {isWeatherLoading ? (lang === 'en' ? 'Fetching...' : '조회 중...') : (
                      weatherData ? `${weatherData.temperature}°C / Wind ${weatherData.windspeed}km/h (${getWeatherDescription(weatherData.weathercode)})` : (lang === 'en' ? 'Weather Data Connected' : '실시간 기상 수신 완료')
                    )}
                  </strong>
                </div>
              </div>
              ...
              <div style={{ marginTop: '0.35rem', paddingTop: '0.3rem', borderTop: '1px solid #fef08a', fontSize: '0.68rem', color: '#78350f', lineHeight: 1.25 }}>
                💡 <strong>{t('aiSummaryLabel')}</strong>: {lang === 'en' ? `Route ${formData.origin.split(' ')[0]} ➔ ${formData.destination.split(' ')[0]} reflects Port Congestion (${portCongestionPct}%) and Open-Meteo weather. Adjusted ETA: ` : `${formData.origin.split(' ')[0]} ➔ ${formData.destination.split(' ')[0]} 항로에서 Marine API 항만 정체(${portCongestionPct}%) 및 Open-Meteo 실시간 기상이 반영되어 ETA가 `}<strong>{vesselRealtimeEta}</strong>{lang === 'en' ? '.' : '로 산출되었습니다.'}
              </div>
            </div>
          </div>
        </div>
